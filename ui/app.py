...
DEPLOY_DEMO_MODE = os.environ.get("DEPLOY_DEMO_MODE", "false").lower() == "true"
...
def summarize_cluster():
    if DEPLOY_DEMO_MODE:
        return {"running": 12, "error": 1, "pending": 1, "total": 14}
    ...
    if "connection refused" in lowered or "unable to connect" in lowered or "error from server" in lowered:
        return {"running": "-", "error": "-", "pending": "-", "total": "-"}

    lines = [l for l in pods_output.strip().split("\n")[1:] if l.strip()]
    running = sum(1 for l in lines if "Running" in l)
    error = sum(1 for l in lines if "Error" in l or "CrashLoopBackOff" in l or "ImagePullBackOff" in l or "ErrImagePull" in l)
    pending = sum(1 for l in lines if "Pending" in l)
    total = len(lines)
    return {"running": running, "error": error, "pending": pending, "total": total}
...
def demo_response(prompt_text: str):
...
"""
    if "pending-pod" in q or "pending" in q:
        return """EVIDENCE
...
- broken-service is a selector and endpoints fault
...
- Deploy workloads to staging if that namespace is intended to be active
...
- kubectl get pods -n staging
"""
    if "what commands did you use" in q:
        return """EVIDENCE
...
ROOT CAUSE
The assistant records diagnostic commands for transparency.

FIX
- Review the commands above to understand how the diagnosis was produced

VERIFY
- kubectl get pods -o wide
"""
    return """EVIDENCE
- This deployed demo currently supports the main Kubernetes diagnosis questions
- Live cluster access is available in the local version of the app

ROOT CAUSE
This public deployment is running in demo mode and is limited to the main supported diagnosis flows.

FIX
- Ask one of the supported demo questions shown in the sidebar
...
issue_text = f"{cluster['error'] + cluster['pending']} currently visible runtime issues found from live pod state." if isinstance(cluster["error"], int) else "Live cluster status unavailable."

with st.sidebar:
    st.markdown("""
    <div class="soft-card">
        <div style="font-size:1.65rem;font-weight:800;">☸️ K8s AI Ops</div>
...
cards = [
    ("Services", "11", "Online Boutique"),
    ("Faults", "4", "Injected"),
    ("Namespaces", "2", "default + staging"),
    ("Model", "Llama 3.3", "Groq Free"),
]
...
main_col, right_col = st.columns([3, 1.0], gap="large")

with right_col:
    st.markdown("""
    <div class="soft-card">
        <div class="section-label" style="margin-bottom:8px;">Quick Summary</div>
        <div class="fault-item">• Demo-safe public deployment</div>
        <div class="fault-item">• Localhost version uses live cluster</div>
        <div class="fault-item">• Command transparency included</div>
    </div>
    """, unsafe_allow_html=True)
...
with main_col:
    if not st.session_state.messages:
        ...
        <div class="welcome-card">
            ...
            <div class="welcome-sub">Ask in plain English or use the quick questions on the left.</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    def handle_prompt(prompt_text: str):
        ...
        if DEPLOY_DEMO_MODE:
            response = demo_response(prompt_text)
        else:
